...
export default async function SubcategoryPage({ params, searchParams }: PageProps) {
  ...
  if (!(DEFAULT_APPLIED_PARAM in search) && isLeafItem("electricals", categoryName)) {
    ...
    redirect(`/products/items/electricals/${subcategory}?${query.toString()}`);
  }

  return (
    <ProductListing
      category={categoryName}
      breadcrumb={getFolderBreadcrumb("electricals", categoryName)}
    />
  );
}
