...
export default async function SubcategoryPage({ params, searchParams }: PageProps) {
  ...
  if (!(DEFAULT_APPLIED_PARAM in search) && isLeafItem("power-hand-tools", categoryName)) {
    ...
    redirect(`/products/items/power-hand-tools/${subcategory}?${query.toString()}`);
  }

  return (
    <ProductListing
      category={categoryName}
      breadcrumb={getFolderBreadcrumb("power-hand-tools", categoryName)}
    />
  );
}
