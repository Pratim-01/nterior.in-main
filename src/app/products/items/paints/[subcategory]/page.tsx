...
export default async function SubcategoryPage({ params, searchParams }: PageProps) {
  ...
  if (!(DEFAULT_APPLIED_PARAM in search) && isLeafItem("paints", categoryName)) {
    ...
    redirect(`/products/items/paints/${subcategory}?${query.toString()}`);
  }

  return (
    <ProductListing
      category={categoryName}
      breadcrumb={getFolderBreadcrumb("paints", categoryName)}
    />
  );
}
