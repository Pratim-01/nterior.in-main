...
export default async function SubcategoryPage({ params, searchParams }: PageProps) {
  ...
  if (!(DEFAULT_APPLIED_PARAM in search) && isLeafItem("kitchen", categoryName)) {
    ...
    redirect(`/products/items/kitchen/${subcategory}?${query.toString()}`);
  }

  return (
    <ProductListing
      category={categoryName}
      breadcrumb={getFolderBreadcrumb("kitchen", categoryName)}
    />
  );
}
